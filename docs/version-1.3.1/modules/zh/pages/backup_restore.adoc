...
*警告*：对于 rke2 v.1.20.9 及之前的所有版本，由于存在一个已知问题，即引导数据可能无法在恢复时保存（步骤 1 -下面的 3 假定了这种情况），因此你需要先备份和恢复证书。请参阅下面的xref:backup_restore.adoc#_恢复快照的其他注意事项[注意事项]，了解特定版本的恢复注意事项。
...
. 你可以根据标准 xref:install/ha.adoc#_3_启动其他_server_节点[RKE2 HA 安装文档]继续向集群添加新的 Server 和 Worker 节点。
