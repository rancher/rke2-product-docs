...
本文档是 RKE2 安全强化指南的补充。安全强化指南为 RKE2 生产环境的安装加固提供了具体指导，而本基准指南旨在帮助您依据 CIS Kubernetes 基准中的每项控制评估加固集群的安全级别。它供 RKE2 操作员、安全团队、审计员和决策者使用。

本指南专用于 RKE2 的 *v1.29-1.34* 版本线以及 CIS Kubernetes 基准的 *v1.11* 版本。

关于每项控制的更多信息，包括详细的理由和检查描述，您可以参考 CIS Kubernetes 基准 v1.8 的相应章节。您可以在 https://www.cisecurity.org/benchmark/kubernetes/[互联网安全中心 (CIS)] 创建免费账户后下载基准。

=== 测试控制方法

CIS Kubernetes 基准中的每项控制均是在根据附带的安全强化指南配置的 RKE2 集群上进行评估的。

每项控制的可能结果如下：

* *PASS* - 该控制已自动化（得分：真）。被测试的 RKE2 集群通过了基准中概述的审计。
* *不适用* - 由于 RKE2 的设计方式，该控制不适用（类型：跳过）。理由部分将解释原因。
* *WARN* - 在 CIS 基准中，该控制为手动（得分：假），并依赖于手动操作员干预。补救部分将提供如何达到通过结果的指导。
...
请在控制平面节点上运行以下命令（依据您系统上的文件位置）。
例如，`chmod 600 /var/lib/rancher/rke2/agent/pod-manifests/kube-apiserver.yaml`
======
...
在控制平面节点上运行以下命令（根据您系统上的文件位置）。
例如，`chmod 600 /var/lib/rancher/rke2/agent/pod-manifests/kube-controller-manager.yaml`
======
...
在控制平面节点上运行以下命令（根据您系统上的文件位置）。
例如，`chmod 600 /var/lib/rancher/rke2/agent/pod-manifests/kube-scheduler.yaml`
======
...
如果仅运行主节点而没有 etcd 角色，则此检查不适用。
如果控制平面和 etcd 角色存在于同一节点上，但此检查为警告，则请在控制平面节点上运行以下命令（依据您系统上的文件位置）。
例如，`chmod 600 /var/lib/rancher/rke2/agent/pod-manifests/etcd.yaml`
======
...
运行以下命令（根据上面找到的 etcd 数据目录）。例如，`chmod 700 /var/lib/rancher/rke2/server/db/etcd`
======
...
如果控制平面和 etcd 角色在同一节点上，但此检查为警告，则在控制平面节点上运行以下命令（根据您系统上的文件位置）。
如果控制平面和 etcd 角色在同一节点上，但此检查为警告，则在 etcd 服务器节点上，获取 etcd 数据目录，作为参数 --data-dir，从命令 'ps -ef | grep etcd' 中获取。
运行以下命令（根据上面找到的 etcd 数据目录）。
例如，`chown etcd:etcd /var/lib/rancher/rke2/server/db/etcd`
======
...
在控制平面节点上运行以下命令（根据您系统上的文件位置）。
例如，`chmod 600 /var/lib/rancher/rke2/server/cred/admin.kubeconfig`
======
...
在控制平面节点上运行以下命令（根据您系统上的文件位置）。
例如，`chown root:root /var/lib/rancher/rke2/server/cred/admin.kubeconfig`
======
...
在控制平面节点上运行以下命令（根据您系统上的文件位置）。
例如，`chmod 600 /var/lib/rancher/rke2/server/cred/scheduler.kubeconfig`
======
...
在控制平面节点上运行以下命令（根据您系统上的文件位置）。
例如，`chown root:root /var/lib/rancher/rke2/server/cred/scheduler.kubeconfig`
======
...
在控制平面节点上运行以下命令（根据您系统上的文件位置）。
例如，`chmod 600 /var/lib/rancher/rke2/server/cred/controller.kubeconfig`
======
...
在控制平面节点上运行以下命令（根据您系统上的文件位置）。
例如，`chown root:root /var/lib/rancher/rke2/server/cred/controller.kubeconfig`
======
...
在控制平面节点上运行以下命令（根据您系统上的文件位置）。
例如，`chown -R root:root /var/lib/rancher/rke2/server/tls`
======
...
在控制平面节点上运行以下命令（根据您系统上的文件位置）。
例如，`chmod -R 644 /var/lib/rancher/rke2/server/tls/*.crt` 默认情况下，RKE2 将 PKI 证书文件的权限设置为 644，支持更严格的权限，例如 600。
======
...
在控制平面节点上运行以下命令（根据您系统上的文件位置）。
例如，`chmod -R 600 /var/lib/rancher/rke2/server/tls/*.key`
======
...
默认情况下，RKE2 将 --anonymous-auth 参数设置为 false。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml 并去除类似以下内容。
...
*修正：*
默认情况下，RKE2 不会设置 DenyServiceExternalIPs。
要启用此标志，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，如下所示。
...
默认情况下，RKE2 自动提供 kubelet 客户端证书和密钥。
它们在 /var/lib/rancher/rke2/server/tls/client-kube-apiserver.crt 和 /var/lib/rancher/rke2/server/tls/client-kube-apiserver.key 生成并存放。如果出于某种原因需要提供自己的证书和密钥，可以在 RKE2 配置文件 /etc/rancher/rke2/config.yaml 中设置以下参数。
...
默认情况下，RKE2 自动提供 kubelet CA 证书文件，位于 /var/lib/rancher/rke2/server/tls/server-ca.crt。
如果出于某种原因需要提供自己的 CA 证书，请考虑使用 rke2 证书命令行工具。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml 并去除以下类似的行。
...
默认情况下，RKE2 不会将 --authorization-mode 设置为 AlwaysAllow。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，去除以下类似的行。
...
默认情况下，RKE2 将 --authorization-mode 设置为 Node 和 RBAC。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，确保没有覆盖 authorization-mode。
======
...
然后，编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml 并设置以下参数。
...
默认情况下，RKE2 不将 --enable-admission-plugins 设置为 AlwaysAdmit。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，去除以下类似的行。
...
在控制平面节点上编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，并将 --enable-admission-plugins 参数设置为包含 AlwaysPullImages。--enable-admission-plugins=...,AlwaysPullImages,...
...
默认情况下，RKE2 不将 --disable-admission-plugins 设置为任何值。
...
默认情况下，RKE2 将 --enable-admission-plugins 设置为 NodeRestriction。
检查 RKE2 配置文件 /etc/rancher/rke2/config.yaml，并确保您没有覆盖 admission 插件。
...
默认情况下，RKE2 将 --profiling 参数设置为 false。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml 并去除以下类似的行。
...
默认情况下，RKE2 将 --audit-log-path 参数设置为 /var/lib/rancher/rke2/server/logs/audit.log。如果您想更改此设置，请在控制平面节点上编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，并将 --audit-log-path 参数设置为适当的路径和文件，以便将审计日志写入，例如，
...
默认情况下，RKE2 将 --audit-log-maxage 参数设置为 30 天。
如果您想更改此设置，请在控制平面节点上编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，并将 --audit-log-maxage 参数设置为适当的天数，例如，
...
默认情况下，RKE2 将 --audit-log-maxbackup 参数设置为 10。
如果您想更改此设置，请在控制平面节点上编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，并将 --audit-log-maxbackup 参数设置为适当的值。
例如，
...
默认情况下，RKE2 将 --audit-log-maxsize 参数设置为 100 MB。
如果您想更改此设置，请在控制平面节点上编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，并将 --audit-log-maxsize 参数设置为合适的大小（以 MB 为单位）。
例如，
...
编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，并在需要时设置以下参数。例如，
...
默认情况下，RKE2 不设置 --service-account-lookup 参数。
编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml 并设置 service-account-lookup。例如，
...
RKE2 会自动生成并设置服务账户密钥文件。
它位于 /var/lib/rancher/rke2/server/tls/service.key。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml 并去除以下类似的行。
...
RKE2 会自动生成并设置 etcd 证书和密钥文件。
它们位于 /var/lib/rancher/rke2/server/tls/etcd/client.crt 和 /var/lib/rancher/rke2/server/tls/etcd/client.key。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml 并去除以下类似的行。
...
默认情况下，RKE2 会自动生成并提供 apiserver 的 TLS 证书和私钥。
它们已生成并位于 /var/lib/rancher/rke2/server/tls/serving-kube-apiserver.crt 和 /var/lib/rancher/rke2/server/tls/serving-kube-apiserver.key。如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml 并去除以下类似的行。
...
默认情况下，RKE2 自动提供客户端证书颁发机构文件。
它已生成并位于 /var/lib/rancher/rke2/server/tls/client-ca.crt。
如果出于某种原因需要提供自己的 CA 证书，请考虑使用 rke2 证书命令行工具。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml 并去除以下类似的行。
...
==== 1.2.27 确保 --encryption-provider-config 参数设置正确（自动）
...
RKE2 始终配置为加密密文。
密文加密通过 rke2 secrets-encrypt 命令行工具进行管理。
如有需要，您可以在 /var/lib/rancher/rke2/server/cred/encryption-config.json 找到生成的加密配置。
======

==== 1.2.28 确保加密提供程序配置正确（自动）
...
RKE2 始终配置为使用 aescbc 加密提供程序来加密密文。
密文加密通过 rke2 secrets-encrypt 命令行工具进行管理。
如有需要，您可以在 /var/lib/rancher/rke2/server/cred/encryption-config.json 找到生成的加密配置。
======
...
默认情况下，RKE2 kube-apiserver 符合此测试。更改这些值可能会导致回归，因此在生产部署中应用之前，请确保所有 apiserver 客户端支持新的 TLS 配置。
如果需要自定义 TLS 配置，请考虑创建与您的要求相符的此规则的自定义版本。
如果此检查失败，请删除与 `tls-cipher-suites` 相关的任何自定义配置，或通过添加以下内容更新 /etc/rancher/rke2/config.yaml 文件以匹配默认值： +
kube-apiserver-arg:
...
编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，并在重新加载 rke2 之前将 --service-account-extend-token-expiration 参数设置为 false，如下所示： +
 kube-apiserver-arg:
...
默认情况下，RKE2 设置 terminated-pod-gc-threshold 为 1000。
如果您需要更改此值，请在控制平面节点上编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，并将 --terminated-pod-gc-threshold 设置为适当的阈值，
...
默认情况下，RKE2 将 --profiling 参数设置为 false。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml 并删除以下类似的行。
...
默认情况下，RKE2 将 --use-service-account-credentials 参数设置为 true。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml 并删除以下类似的行。
...
默认情况下，RKE2 自动提供服务帐户私钥文件。
它是在 /var/lib/rancher/rke2/server/tls/service.current.key 生成并存放的。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml 并删除以下类似的行。
...
默认情况下，RKE2 自动提供根 CA 文件。
它在 /var/lib/rancher/rke2/server/tls/server-ca.crt 生成并存放。
如果出于某种原因您需要提供自己的 CA 证书，请考虑使用 rke2 证书命令行工具。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml 并删除以下类似的行。
...
默认情况下，RKE2 不会设置 RotateKubeletServerCertificate 功能开关。
如果您启用了此功能开关，您应该将其去除。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，删除以下类似的行。
...
默认情况下，RKE2 将 --bind-address 参数设置为 127.0.0.1。如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，并删除以下类似的行。
...
默认情况下，RKE2 为 etcd 生成证书和密钥文件。
这些文件位于 /var/lib/rancher/rke2/server/tls/etcd/。
如果此检查失败，请确保配置文件 /var/lib/rancher/rke2/server/db/etcd/config 未被修改为使用自定义证书和密钥文件。
======
...
默认情况下，RKE2 将 --client-cert-auth 参数设置为 true。
如果此检查失败，请确保配置文件 /var/lib/rancher/rke2/server/db/etcd/config 未被修改为禁用客户端证书认证。
======
...
默认情况下，RKE2 不设置 --auto-tls 参数。
如果此检查失败，请在主节点上编辑 etcd pod 规格文件 /var/lib/rancher/rke2/server/db/etcd/config，去除 --auto-tls 参数或将其设置为 false。client-transport-security: auto-tls: false
======
...
默认情况下，RKE2 为 etcd 生成对等证书和密钥文件。
这些文件位于 /var/lib/rancher/rke2/server/tls/etcd/。
如果此检查失败，请确保配置文件 /var/lib/rancher/rke2/server/db/etcd/config 未被修改为使用自定义对等证书和密钥文件。
======
...
默认情况下，RKE2 将 --peer-cert-auth 参数设置为 true。
如果此检查失败，请确保配置文件 /var/lib/rancher/rke2/server/db/etcd/config 未被修改为禁用对等客户端证书认证。
======
...
默认情况下，RKE2 不设置 --peer-auto-tls 参数。
如果此检查失败，请在主节点上编辑 etcd pod 规格文件 /var/lib/rancher/rke2/server/db/etcd/config，去除 --peer-auto-tls 参数或将其设置为 false。peer-transport-security: auto-tls: false
======
...
默认情况下，RKE2 为 etcd 生成一个唯一的证书颁发机构。
该文件位于 /var/lib/rancher/rke2/server/tls/etcd/peer-ca.crt。
如果此检查失败，请确保配置文件/var/lib/rancher/rke2/server/db/etcd/config 未被修改为使用共享的证书颁发机构。
======
...
*修正：*
Kubernetes 提供的替代机制，如使用 OIDC，应替代客户端证书。

==== 3.1.2 服务账户令牌认证不应用于用户（手动）

*结果：*警告

*修正：*
应实施 Kubernetes 提供的替代机制，例如使用 OIDC，来替代服务账户令牌。

==== 3.1.3 启动令牌认证不应用于用户（手动）

*结果：*警告

*修正：*
应实施 Kubernetes 提供的替代机制，例如使用 OIDC，来替代启动令牌。
...
*修正：*
审查为集群提供的审计策略，并确保它至少涵盖以下领域，

* 对集群管理的机密的访问。应谨慎处理，仅记录对机密、配置映射和令牌审查的请求的元数据，以避免记录敏感数据的风险。
* 对 Pod 和部署对象的修改。
* 使用 `pods/exec`、`pods/portforward`、`pods/proxy` 和 `services/proxy`。
对于大多数请求，建议至少在元数据级别进行日志记录（最基本的日志记录级别）。
...
kubelet 由 RKE2 进程管理。没有 kubelet 服务文件，所有配置在运行时作为参数传递。
...
例如，`chmod 600 /var/lib/rancher/rke2/agent/kubeproxy.kubeconfig`
======
...
在每个工作节点上运行以下命令（根据您系统上的文件位置）。
例如，`chmod 600 /var/lib/rancher/rke2/agent/kubelet.kubeconfig`
======
...
在每个工作节点上运行以下命令（根据您系统上的文件位置）。
例如，`chmod 600 /var/lib/rancher/rke2/agent/client-ca.crt`
======
...
kubelet 由 RKE2 进程管理。没有 kubelet 配置文件，所有配置在运行时作为参数传递。
...
*预期结果：* '.authentication.anonymous.enabled' 等于 'false'
...
默认情况下，RKE2 将 --anonymous-auth 设置为 false。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，删除任何类似于以下内容的行。
...
默认情况下，RKE2 不会将 --authorization-mode 设置为 AlwaysAllow。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，去除任何类似于以下内容的行。
...
*预期结果：* '.authentication.x509.clientCAFile' 存在
...
默认情况下，RKE2 会自动为 Kubelet 提供客户端 CA 证书。
它是在 /var/lib/rancher/rke2/agent/client-ca.crt 生成并存放的。
======
...
默认情况下，RKE2 将 --read-only-port 设置为 0。如果您将此设置为其他值，您应该将其恢复为 0。编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，去除任何类似于以下内容的行。
...
*预期结果：* '.streamingConnectionIdleTimeout' 不等于 '0' 或 '.streamingConnectionIdleTimeout' 不存在
...
*预期结果：* '.makeIPTablesUtilChains' 存在或 '.makeIPTablesUtilChains' 不存在
...
或者，去除 --make-iptables-util-chains 参数，让 RKE2 使用默认值。
根据您的系统，重启 RKE2 服务。例如，systemctl restart rke2-server.service
======
...
默认情况下，RKE2 会设置 --hostname-override 参数。根据 CIS 指南，这是为了遵守要求此标志的云服务提供商，以确保主机名与节点名称匹配。
...
*预期结果：* '.eventRecordQPS' 存在或 '.eventRecordQPS' 不存在
...
*预期结果：* '.tlsCertFile' 存在 且 '.tlsPrivateKeyFile' 存在
...
默认情况下，RKE2 会自动提供 Kubelet 的 TLS 证书和私钥。
它们被生成并位于 /var/lib/rancher/rke2/agent/serving-kubelet.crt 和 /var/lib/rancher/rke2/agent/serving-kubelet.key。如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，并去除任何类似于以下内容的行。
...
*预期结果:* '.rotateCertificates' 存在或 '.rotateCertificates' 不存在
...
默认情况下，RKE2 不会设置 --rotate-certificates 参数。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，去除任何 rotate-certificates 参数。
根据您的系统，重启 RKE2 服务。例如，systemctl restart rke2-server.service
======
...
*预期结果:* '.featureGates.RotateKubeletServerCertificate' 存在 或 '.featureGates.RotateKubeletServerCertificate' 不存在
...
默认情况下，RKE2 不会设置 RotateKubeletServerCertificate 功能开关。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，去除任何 RotateKubeletServerCertificate 参数。
根据您的系统，重启 RKE2 服务。例如，systemctl restart rke2-server.service
======
...
*修正：*
编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，
...
如果使用 RKE2 配置文件 /etc/rancher/rke2/config.yaml，请编辑该文件以将 `seccomp-default` 设置为 kubelet-arg:
...
*预期结果：* '--metrics-bind-address' 存在或者 '--metrics-bind-address' 不存在
...
[cols=2*]
|===
| *预期结果：* 'cluster-admin' 被正则表达式 'cluster-admin' 匹配
...
识别所有与 cluster-admin 角色相关的 clusterrolebindings。检查它们是否被使用，以及它们是否需要此角色，或者是否可以使用具有更少权限的角色。RKE2 对 helm-kube-system-rke2-* clusterrolebindings 给予例外，这些角色绑定处理所有 rke2 管理组件的安装。
在可能的情况下，首先将用户绑定到权限较低的角色，然后移除与 cluster-admin 角色的 clusterrolebinding：
...
RKE2 对以下集群角色给予例外，这些角色是常规操作所必需的：
...
修改每个默认服务帐户的配置以包含此值 automountServiceAccountToken: false 或使用 kubectl:
...
修改不需要挂载服务账户令牌的 ServiceAccounts 和 Pods 的定义，以禁用它，使用 `automountServiceAccountToken: false`。
RKE2 对 kube-system 命名空间中的 ServiceAccounts 给予例外，因为这些是预期与 apiserver 联系的。
在 cis 模式下，RKE2 禁用命名空间的 automountServiceAccountToken：

* kube-public, default, kube-node-lease。如果 ServiceAccount 和 Pod 的 .spec 都指定了 automountServiceAccountToken 的值，则 Pod 的 spec 优先。
条件：当 Pod 的 is_compliant 为 true 时
 ** 服务账户的 automountServiceAccountToken: false 且 Pod 的 automountServiceAccountToken: false 或未设置
 ** 服务账户的 automountServiceAccountToken: true/未设置 且 Pod 的 automountServiceAccountToken: false
======
...
RKE2 对以下 Pod 给予例外，这些 Pod 是正常操作所需的：

* kube-flannel，calico-node，kube-proxy 审计：审计列出所有 Pod 的容器以获取它们的 .securityContext.privileged 值。
...
RKE2 对 kube-system 名称空间中的以下 Pod 给予例外，这些 Pod 是正常操作所需的：

* cloud-controller-manager，etcd，rke2-canal，rke2-coredns，kube-apiserver，
* kube-scheduler，kube-proxy，kube-controller-manager 审计：审计获取每个 Pod 的 spec.hostNetwork。
条件：如果 Pod 的 spec.hostNetwork 设置为 `true`，则 is_compliant 为 false。
默认：默认情况下，创建 hostNetwork 容器没有限制。
======
...
为集群中每个包含用户工作负载的名称空间添加策略，以限制允许特权提升的容器的准入，其中 `.securityContext.allowPrivilegeEscalation` 设置为 `true`。
...
默认值：如果未设置，则允许特权提升（默认为 true）。但是，如果使用带有 `restricted` 控制文件的 PSP/PSA，则特权提升被明确禁止，除非另行配置。
======
...
*修正:*
为集群中具有用户工作负载的每个名称空间添加策略，以限制具有 `NET_RAW` 能力的容器的接纳。
...
* rke2-ingress-ngninx-controller，coredns 审计：审计获取每个 Pod 中容器已添加的能力。
条件：如果给定容器添加了附加能力，则 is_compliant 为 false。
...
*修正:*
遵循Kubernetes文档并将SecurityContexts应用于您的Pods。有关建议的 SecurityContexts 列表，您可以参考 CIS Docker 容器安全基准。
