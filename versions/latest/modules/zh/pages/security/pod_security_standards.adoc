...
.版本
====
本文档适用于 RKE2 v1.25 及更新版本，请参阅 xref:security/pod_security_policies.adoc[Pod 安全策略文档]了解 RKE2 v1.24 及更低版本的默认策略。
====
...
* 如果使用 `--profile=cis-1.23` 选项运行，RKE2 将通过配置文件应用受限的 Pod 安全标准，该配置文件将在整个集群中强制执行 `restricted` 模式，其中 `kube-system`、`compliance-operator-system` 和 `tigera-operator` 命名空间除外，它们用于确保系统 Pod 能成功运行。
