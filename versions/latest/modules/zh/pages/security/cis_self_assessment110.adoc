...
本指南特定于 RKE2 的 *v1.28-1.31* 版本线和 CIS Kubernetes 基准的 *v1.10* 版本。
...
如果您想更改此设置，请在控制平面节点上编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，并将 --audit-log-maxsize 参数设置为合适的 MB 大小。
例如，
...
它们生成并位于 /var/lib/rancher/rke2/server/tls/serving-kube-apiserver.crt 和 /var/lib/rancher/rke2/server/tls/serving-kube-apiserver.key。如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml 并去除以下类似的行。
...
它生成并位于 /var/lib/rancher/rke2/server/tls/client-ca.crt。
如果出于某种原因需要提供自己的 CA 证书，请考虑使用 rke2 证书命令行工具。
如果此检查失败，请编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml 并去除以下类似的行。
...
RKE2 始终配置为加密机密。
机密加密通过 rke2 secrets-encrypt 命令行工具进行管理。
...
RKE2 始终配置为使用 aescbc 加密提供程序来加密机密。
机密加密通过 rke2 secrets-encrypt 命令行工具进行管理。
如有需要，您可以在 /var/lib/rancher/rke2/server/cred/encryption-config.json 找到生成的加密配置。
======
...
如果此检查失败，请删除与 `tls-cipher-suites` 相关的任何自定义配置，或通过添加以下内容更新 /etc/rancher/rke2/config.yaml 文件以匹配默认值：kube-apiserver-arg：
...
*预期结果：*'ETCD_AUTO_TLS' 不存在或 'ETCD_AUTO_TLS' 存在或 '.client-transport-security.auto-tls' 存在
...
默认情况下，RKE2 不会设置 --auto-tls 参数。
如果此检查失败，请在主节点上编辑 etcd pod 规格文件 /var/lib/rancher/rke2/server/db/etcd/config，删除 --auto-tls 参数或将其设置为 false。client-transport-security: auto-tls: false
======
...
*预期结果：*'ETCD_PEER_CLIENT_CERT_AUTH' 存在或 '.peer-transport-security.client-cert-auth' 等于 'true'
...
*预期结果：*'ETCD_PEER_AUTO_TLS' 不存在或 'ETCD_PEER_AUTO_TLS' 存在或 '.peer-transport-security.auto-tls' 存在
...
默认情况下，RKE2 不会设置 --peer-auto-tls 参数。
如果此检查失败，请在主节点上编辑 etcd pod 规格文件 /var/lib/rancher/rke2/server/db/etcd/config，删除 --peer-auto-tls 参数或将其设置为 false。peer-transport-security: auto-tls: false
======
...
*预期结果：*'ETCD_TRUSTED_CA_FILE' 存在或 '.peer-transport-security.trusted-ca-file' 等于 '/var/lib/rancher/rke2/server/tls/etcd/peer-ca.crt'
...
该证书位于 /var/lib/rancher/rke2/server/tls/etcd/peer-ca.crt。
如果此检查失败，请确保配置文件 /var/lib/rancher/rke2/server/db/etcd/config 未被修改为使用共享证书颁发机构。
======
...
*修正：*
应使用 Kubernetes 提供的替代机制，如 OIDC，来替代客户端证书。

==== 3.1.2 不应为用户使用服务账户词元认证（手动）

*结果：*警告

*修正：*
应使用 Kubernetes 提供的替代机制，如 OIDC，来替代服务账户词元。

==== 3.1.3 不应为用户使用启动词元身份验证（手动）

*结果：*警告

*修正：*
应实施 Kubernetes 提供的替代机制，例如使用 OIDC，来替代启动词元。
...
*修正：*
审查为集群提供的审计策略，并确保其至少涵盖以下领域，

* 对集群管理的机密的访问。应谨慎处理，仅记录对机密、配置映射和词元审查的请求的元数据，以避免记录敏感数据的风险。
* 对 Pod 和部署对象的修改。
...
编辑 RKE2 配置文件 /etc/rancher/rke2/config.yaml，将以下参数设置为适当值。
...
或者，删除 --make-iptables-util-chains 参数，让 RKE2 使用默认值。
根据您的系统，重启 RKE2 服务。例如， +
systemctl restart rke2-server.service
======
...
它们是在 /var/lib/rancher/rke2/agent/serving-kubelet.crt 和 /var/lib/rancher/rke2/agent/serving-kubelet.key 生成并存放的。 +
...
识别所有与 cluster-admin 角色相关的 clusterrolebindings。检查它们是否被使用，以及它们是否需要此角色，或者是否可以使用具有更少权限的角色。RKE2 对 helm-kube-system-rke2-* clusterrolebindings 给予例外，这些角色处理所有 rke2 管理组件的安装。
...
RKE2 对 kube-system 名称空间中的 ServiceAccounts 给予例外，因为这些账户预计会联系 apiserver。
在 cis 模式下，RKE2 禁用以下名称空间的 automountServiceAccountToken：
...
RKE2 对以下 Pod 给予豁免，这些 Pod 是正常操作所需的：
...
* rke2-ingress-ngninx-controller，coredns 审计：审计获取每个 Pod 的容器所添加的能力。
