...
CoreDNS is deployed with the https://github.com/kubernetes-incubator/cluster-proportional-autoscaler[autoscaler] by default. To disable it or change its config, use the xref:add-ons/helm.adoc#_使用_helmchartconfig_自定义打包组件[HelmChartConfig] resource.
...
Configuration options can be specified by creating a xref:add-ons/helm.adoc#_使用_helmchartconfig_自定义打包组件[HelmChartConfig manifest] to customize the `rke2-ingress-nginx` HelmChart values. For example, a HelmChartConfig at `/var/lib/rancher/rke2/server/manifests/rke2-ingress-nginx-config.yaml` with the following contents sets `use-forwarded-headers` to `"true"` in the ConfigMap storing the NGINX config:
...
Configuration options can be specified by creating a xref:add-ons/helm.adoc#_使用_helmchartconfig_自定义打包组件[HelmChartConfig manifest] to customize the `rke2-traefik` HelmChart values. For example, a HelmChartConfig at `/var/lib/rancher/rke2/server/manifests/rke2-traefik-config.yaml` with the following contents changes the log level to "DEBUG":
