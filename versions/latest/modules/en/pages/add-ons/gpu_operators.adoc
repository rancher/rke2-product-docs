...
The operator then relocates the modules to the node's running kernel path using SUSE's stable kABI guarantee.
As a result, a single driver image works for any kernel update within the same codestream without requiring driver recompilation.
