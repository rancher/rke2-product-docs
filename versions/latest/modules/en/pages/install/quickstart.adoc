= Quick Start
:description: Launch a cluster with default options by installing RKE2 on server and agent nodes using the provided installation script and configuration steps
...
. Set the correct Prime configuration. 
+
If using the 2026 May releases (or newer): 1.36.1+rke2r2, 1.35.5+rke2r2, 1.34.8+rke2r2, 1.33.11+rke2r2. Open the xref:install/configuration.adoc[RKE2 configuration file] on all server nodes and enable the `prime` flag:
+
[,yaml]
----
...
----
+
This configuration sets `registry.rancher.com` as the default location for all container images and points at the `ingress-nginx` supported image for `ingress-nginx` users
+
If using the 2026 April releases (or older): v1.35.3+rke2r3, v1.34.6+rke2r3, v1.33.10+rke2r3. Open the xref:install/configuration.adoc[RKE2 configuration file] on all server nodes and set the `system-default-registry`:
+
[,yaml]
----
# /etc/rancher/rke2/config.yaml
...
[#_linux_agent_worker_node_installation]
...
[#_windows_agent_worker_node_installation]
...
----
+
To learn more about the config.yaml file, refer to the xref:install/configuration.adoc#_configuration_file[Configuration Options documentation].
