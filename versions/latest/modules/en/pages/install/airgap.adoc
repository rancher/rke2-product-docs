= Air-Gap Install
:description: Install RKE2 in an air-gapped environment by loading required images, meeting prerequisites, and following steps to configure SELinux and install RPM dependencies
...
+
The xref:install/methods.adoc#_rpm[RPM install method] assumes network access to the package repository. On an air-gapped host, you must obtain the RPM and its dependencies on a machine that can reach the repository, copy the packages to the offline node, then install them with `rpm`, `dnf`, or `yum` from the local files.
