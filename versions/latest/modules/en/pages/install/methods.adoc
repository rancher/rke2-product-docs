...
Run the installer, where `INSTALL_RKE2_ARTIFACT_URL` is the Prime Artifacts URL and `INSTALL_RKE2_CHANNEL` is the most recent stable version of RKE2. If you want to specify a version, set the `INSTALL_RKE2_CHANNEL` environment variable.

[,bash]
----
sudo INSTALL_RKE2_ARTIFACT_URL=<PRIME-ARTIFACTS-URL>/rke2 INSTALL_RKE2_CHANNEL="latest" INSTALL_RKE2_CHANNEL=latest ./install.sh
----
