...
[WARNING]
====
Because no other system modifications are made, the tarball method does not install the `rke2-selinux` policy and does not enable SELinux support in RKE2. On a node running SELinux in `Enforcing` mode, xref:security/selinux.adoc#_installing_rke2_selinux_on_its_own[install `rke2-selinux` first], then install RKE2 and set `selinux: true` in the config file.
====
