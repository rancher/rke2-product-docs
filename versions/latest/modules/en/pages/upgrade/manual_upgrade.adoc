= Manual Upgrades
:description: Upgrade RKE2 manually using the installation script, downloading and replacing the binary, or running RPM commands to update the server or agent packages
...
Upgrades performed via the installation script or using our xref:upgrade/automated_upgrade.adoc[automated upgrades] feature can be tied to different release channels. The following channels are available:
...
[#_upgrade_rke2_using_the_installation_script]
...
[#_manually_upgrade_rke2_using_the_binary]
...
[#_upgrade_rke2_using_the_rpm_upgrades]
