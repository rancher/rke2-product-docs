...
* Provides defaults and configuration options that allow clusters to pass CIS Kubernetes Benchmarks with minimal operator intervention. See xref:security/hardening_guide.adoc[CIS Hardening Guide] for the full list of supported benchmark versions.
* Enables xref:security/fips3_support.adoc[FIPS 140-3 compliance].
* Regularly scans components for CVEs using https://github.com/aquasecurity/trivy[trivy] in our build pipeline.

[#_how_is_this_different_from_rke_or_k3s_product_name]
...
SUSE supports responsible disclosure and endeavors to resolve security issues in a reasonable timeframe. To report a security vulnerability, please refer to our https://github.com/rancher/rke2/security/policy[Security Policy].
