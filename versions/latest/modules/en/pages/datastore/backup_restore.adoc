...
The `--name` flag can only be set when running the `rke2 etcd-snapshot save` command. The other two can also be part of the `rke2 server` xref:install/configuration.adoc#_configuration_file[configuration file].
...
[#_s3_compatible_object_store_support]
...
[#_s3_configuration_secret_support]
...
If xref:security/secrets_encryption.adoc[secrets encryption] is enabled, snapshots also include the secrets encryption configuration and keys.
...
[#_etcdsnapshotfile_custom_resources]
...
[#_external_db_backups_experimental]
...
[#_backup_and_restore_with_sqlite]
...
[#_backup_and_restore_with_external_datastore]
