...
:description: Configure a cluster load balancer using HAProxy and Keepalived to manage traffic between server and agent nodes
...
https://www.haproxy.org/[HAProxy] is an open source option that provides a TCP load balancer. It also supports HA for the load balancer itself, ensuring redundancy at all levels. See https://docs.haproxy.org/2.8/intro.html[HAProxy Documentation] for more info.
...
[NOTE]
====
Binding port 6443 is only useful if you want to connect to the API Server through HAProxy.
====
...
Deploy a xref:install/ha.adoc[HA RKE2 cluster with embedded etcd] on the 3 server nodes.
...
For server nodes beyond the first, also set `server: https://10.10.10.100:9345` as shown in the xref:install/ha.adoc[HA installation guide].
