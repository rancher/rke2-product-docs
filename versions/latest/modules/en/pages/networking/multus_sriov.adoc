= Multus
:description: Learn how to use Multus to attach multiple network interfaces to Kubernetes pods by configuring a primary CNI plugin
...
[#_using_multus_with_the_containernetworking_plugins]
...
+
--
Multus provides an optional daemonset to deploy the DHCP daemon required to run the https://www.cni.dev/plugins/current/ipam/dhcp/[DHCP IPAM plugin]. You can do this by using the following xref:add-ons/helm.adoc#_customizing_packaged_components_with_helmchartconfig[HelmChartConfig]:
...
[#_enabling_multus_dynamic_networks_controller]
...
[#_using_multus_with_sr_iov]
== Using Multus with SR-IOV

Using the SR-IOV CNI with Multus can help with data-plane acceleration use cases, providing an extra interface in the pod that can achieve very high throughput. Complete deployment steps, prerequisites, and hardware compatibility details can be found in the https://github.com/k8snetworkplumbingwg/sriov-network-operator/blob/master/doc/quickstart.md[SR-IOV Network Operator Quickstart Guide].

For fully validated configurations and enterprise-grade infrastructure support for SR-IOV in RKE2, refer to https://documentation.suse.com/suse-edge/3.5/html/edge/atip-features.html#sriov[SUSE Telco Cloud].
