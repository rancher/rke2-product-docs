...
CoreDNS is deployed with the https://github.com/kubernetes-incubator/cluster-proportional-autoscaler[autoscaler] by default. To disable it or change its config, use the xref:add-ons/helm.adoc#_customizing_packaged_components_with_helmchartconfig[HelmChartConfig] resource.
...
[#_customizing_the_nodelocal_dns_corefile]
...
[#_nodelocal_dns_cache_with_cilium_in_kube_proxy_replacement_mode]
...
[WARNING]
====
The ingress-nginx controller reaches End of Life (EOL) in March 2026 and is deprecated in RKE2 v1.36. In RKE2 v1.37 ingress-nginx is removed and unavailable for new clusters.
...
Configuration options can be specified by creating a xref:add-ons/helm.adoc#_customizing_packaged_components_with_helmchartconfig[HelmChartConfig manifest] to customize the `rke2-ingress-nginx` HelmChart values. For example, a HelmChartConfig at `/var/lib/rancher/rke2/server/manifests/rke2-ingress-nginx-config.yaml` with the following contents sets `use-forwarded-headers` to `"true"` in the ConfigMap storing the NGINX config:
...
For more information, refer to the official https://github.com/rancher/rke2-charts/tree/main/charts/rke2-ingress-nginx/rke2-ingress-nginx/4.14.400#configuration[ingress-nginx Helm configuration parameters].
...
Configuration options can be specified by creating a xref:add-ons/helm.adoc#_customizing_packaged_components_with_helmchartconfig[HelmChartConfig manifest] to customize the `rke2-traefik` HelmChart values. For example, a HelmChartConfig at `/var/lib/rancher/rke2/server/manifests/rke2-traefik-config.yaml` with the following contents changes the log level to "DEBUG":
  ...
  valuesContent: |-
    logs:
      general:
        level: "DEBUG"
----
To see all configurable options, visit the chart's https://github.com/rancher/rke2-charts/blob/main/charts/rke2-traefik/rke2-traefik/39.0.502/values.yaml[values.yaml].
...
To leverage Gateway API in {rke2-product-name} you must use Traefik. Please use the following HelmChartConfig to enable GatewayAPI:
...
```
* Traefik v3.7.x is compatible with https://gateway-api.sigs.k8s.io/docs/implementations/versions/v1.5/[Gateway API v1.5].
* Traefik v3.6.x is compatible with https://gateway-api.sigs.k8s.io/docs/implementations/versions/v1.4/[Gateway API v1.4].
...
[WARNING]
====
If you need support for experimental Gateway API resources, you must install them separately. For example, for Gateway API v1.4, install the https://github.com/kubernetes-sigs/gateway-api/releases/download/v1.4.0/experimental-install.yaml[v1.4 experimental-install.yaml] from the official https://github.com/kubernetes-sigs/gateway-api/releases/tag/v1.4.0[gateway-api releases] and use the option `providers.kubernetesGateway.experimentalChannel=true` in the rke2-traefik chart values.
====

Starting with RKE2 v1.37, Gateway API CRDs are no longer bundled with Traefik; they are now part of a standalone chart. For more information on this change, see the https://github.com/rancher/rke2/blob/master/docs/adrs/011-bundle-gatewayAPI.md#standalone-gateway-api-crd-management[Architectural Decision Record].
...
[#_deploying_an_external_cloud_controller_manager]
== Deploying an External Cloud Controller Manager

RKE2 ships an embedded Cloud Controller Manager (CCM) that:

* Hosts the <<_service_load_balancer,ServiceLB>> LoadBalancer controller (when enabled with `--enable-servicelb`).
...
. Start every RKE2 server with `--disable-cloud-controller` so the embedded CCM is not scheduled.
. Deploy the external CCM before nodes are expected to become fully schedulable. Prefer an RKE2 xref:add-ons/helm.adoc[`HelmChart`] with `spec.bootstrap: true` so the chart is applied early enough in cluster bootstrap (the same pattern used for other bootstrap-critical components).
. Configure the CCM for your cloud according to the provider's documentation. Node addresses and provider IDs then come from the cloud instance metadata APIs rather than the RKE2 flag values.
