= Secrets Encryption
:description: Configure secrets encryption to protect data at rest, manage encryption providers, rotate encryption keys, and check encryption status
:page-languages: [en, de, es, fr, ja, pt, zh]
:revdate: 2026-07-24
:page-revdate: {revdate}

[#_secrets_encryption_config]
...
.FIPS Compliance
====
FIPS 140-3 compliance is only available with the `aescbc` provider.
====
...
. Update/Add the `secrets-encryption-provider` flag in the RKE2 configuration file on your server nodes to `secretbox`.
. Sequentially restart RKE2 on the server nodes, to load the new configuration:
+
[,bash]
----
...
[#_generated_encryption_config_file]
...
[#_encryption_key_rotation_classic]
