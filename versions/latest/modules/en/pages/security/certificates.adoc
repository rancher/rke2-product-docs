...
RKE2 client and server certificates are valid for 365 days from their date of issuance. Any certificates that are expired or within 120 days to expire are automatically renewed every time RKE2 starts. This renewal extends the lifetime of the existing certs. If you require new certificates and keys, use the <<Rotating Client and Server Certificates Manually,RKE2 CLI>>.
