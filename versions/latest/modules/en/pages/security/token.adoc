= Token Management
:description: Manage RKE2 tokens to secure node joins, encrypt data, and authenticate cluster connections using secure or short formats
...
[#_rke2_token_generate]
=== `rke2 token generate`

Generate a randomly-generated bootstrap token.

You don't have to use this command in order to generate a token. You can do so yourself as long as it is in the format [6 alphanumeric characters].[16 alphanumeric characters], where the first portion is the token ID, and the second portion is the secret.
