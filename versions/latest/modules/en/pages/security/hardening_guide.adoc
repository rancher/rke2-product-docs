= CIS Hardening Guide
:description: Learn to harden a production SUSE Rancher Prime RKE2 cluster using CIS benchmarks and host-level modifications
...
* xref:security/cis_self_assessment112.adoc[CIS Self-Assessment Guide v1.12] for RKE2 v1.33 and newer
* xref:security/cis_self_assessment111.adoc[CIS Self-Assessment Guide v1.11] for RKE2 v1.29-v1.32
* xref:security/cis_self_assessment110.adoc[CIS Self-Assessment Guide v1.10] for RKE2 v1.28-v1.31
* xref:security/cis_self_assessment19.adoc[CIS Self-Assessment Guide v1.9] for RKE2 v1.27-v1.29
...
[#_kubelet_parameter_protect_kernel_defaults_is_set_to_true]
...
. Configures the etcd static pod to run as the etcd user and group, as explained in the xref:security/hardening_guide#_etcd_is_configured_properly[etcd hardening guide].
...
. Configures the Pod Security Admission Controller to enforce restricted mode in all namespaces, with the exception of the `kube-system`, `compliance-operator-system`, and `tigera-operator` namespaces. These namespaces are exempted to allow system pods to run without restrictions, which is required for proper operation of the cluster. For more information about the PSA configuration, see the default xref:security/pod_security_standards.adoc#_pod_security_standards[Pod Security Admission configurations]. For more information about Pod Security Standards, please refer to the https://kubernetes.io/docs/concepts/security/pod-security-standards/[official documentation].
--
...
[#_kubernetes_runtime_requirements]
...
[#_configure_default_service_account]
...
[#_api_server_audit_configuration]
