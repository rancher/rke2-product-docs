...
:description: Migrate from Ingress NGINX to Traefik as the default ingress controller in RKE2 clusters using step-by-step instructions
...
SUSE Rancher Prime LTS customers will receive Ingress NGINX https://www.suse.com/c/trade-the-ingress-nginx-retirement-for-up-to-2-years-of-rke2-support-stability/[support through November 2027].
====
...
** Any release > v1.35
* While Traefik includes a compatibility layer for interpreting Ingress NGINX annotations, not all annotations are supported. Review the https://doc.traefik.io/traefik/reference/routing-configuration/kubernetes/ingress-nginx/#annotations-support[Traefik & Ingresses with NGINX Annotations documentation] to ensure your existing Ingress resources are compatible. You can also use the https://github.com/traefik/ingress-nginx-migration?tab=readme-ov-file#installation[Traefik Ingress NGINX Annotations Discovery Tool] to help identify any unsupported annotations in your cluster.
...
[#_phase_1_dual_ingress_controller_setup]
...
[#_1_assign_ingressclassname_nginx_to_existing_ingresses]
...
.Verification of IngressClass assignment
[%collapsible]
======
...
====
If you are using the Image Tarball, note that Traefik is not included in the default `rke2-images.linux-amd64.tar.zst` asset (example assuming amd64), and you will need to download the additional `rke2-images-traefik.linux-amd64.tar.zst` tarball, and place it in the corresponding folder on the air gap node.
====

[#_3_configure_traefik_ports_and_compatibility_settings]
...
[#_phase_2_parallel_migration_and_validation]
...
[#_1_duplicate_and_reclassify_ingresses]
...
[#_2_test_services_via_both_controllers]
...
[#_3_optional_configure_external_load_balancer]
...
Refer to the https://doc.traefik.io/traefik/migrate/nginx-to-traefik/#step-3-shift-traffic-to-traefik[Traefik Migration Guide] for either DNS-Based migration or External Load Balancer with Weighted Traffic strategies. Take into account that the guide expects both ingresses to include a service with a LoadBalancer address but this guide is assuming node ports are used.
...
[#_phase_3_final_switchover_and_port_reassignment]
...
[#_2_configure_traefik_for_standard_ports]
...
[#_4_final_verification_on_standard_ports]
...
[#_1_remove_ingress_nginx_objects]
