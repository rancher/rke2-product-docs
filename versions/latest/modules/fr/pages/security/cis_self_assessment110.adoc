= CIS 1.10 Guide d'auto-évaluation
...
Ce guide est spécifique à la ligne de version *v1.28-1.31* de RKE2 et à la version *v1.10* du CIS Kubernetes Benchmark.
...
* *Non applicable* - Le contrôle n'est pas applicable (type{nbsp}: skip) à RKE2 en raison de la manière dont il est conçu pour fonctionner. La section de justification expliquera pourquoi cela est le cas.
...
Exécutez la commande ci-dessous (en fonction du répertoire de données etcd trouvé ci-dessus). Par exemple, `chmod 700 /var/lib/rancher/rke2/server/db/etcd`
======
...
==== 1.1.20 Assurez-vous que les permissions du fichier de certificat de l'infrastructure de clés publiques de Kubernetes sont définies sur 600 ou plus restrictives (Manuel)

*Résultat :* WARN

*Correction:*
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud de plan de contrôle.
Par exemple, `chmod -R 600 /var/lib/rancher/rke2/server/tls/*.crt`

==== 1.1.21 Assurez-vous que les permissions du fichier de clé de l'infrastructure de clés publiques de Kubernetes sont définies sur 600 (Automatisé)
...
Ensuite, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et définissez les paramètres ci-dessous.
...
Par défaut, RKE2 ne définit pas l'argument --disable-admission-plugins sur quoi que ce soit.
...
Par défaut, RKE2 définit l'argument --enable-admission-plugins sur NodeRestriction.
...
==== 1.2.16 Assurez-vous que l'argument --audit-log-path est défini (Automatisé)
...
Si vous souhaitez modifier cela, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml sur le nœud de contrôle et définissez le paramètre --audit-log-maxage sur un nombre de jours approprié, par exemple,
...
Si vous souhaitez modifier cela, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml sur le nœud de contrôle et définissez le paramètre --audit-log-maxbackup sur une valeur appropriée.
Exemples :
...
==== 1.2.20 Assurez-vous que l'argument --request-timeout est défini de manière appropriée (Automatisé)
...
Par défaut, le kube-apiserver de RKE2 respecte ce test. Les modifications apportées à ces valeurs peuvent entraîner des régressions, assurez-vous donc que tous les clients apiserver prennent en charge la nouvelle configuration TLS avant de l'appliquer dans des déploiements en production.
...
Si cette vérification échoue, supprimez toute configuration personnalisée autour de `tls-cipher-suites` ou mettez à jour le fichier /etc/rancher/rke2/config.yaml pour correspondre à la configuration par défaut en ajoutant ce qui suit{nbsp}: kube-apiserver-arg{nbsp}:
...
Par défaut, RKE2 définit un seuil de collecte des pods terminés de 1000.
Si vous devez modifier cette valeur, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml sur le nœud de plan de contrôle et définissez le --terminated-pod-gc-threshold à un seuil approprié,
...
Si cette vérification échoue, assurez-vous que le fichier de configuration /var/lib/rancher/rke2/server/db/etcd/config n'a pas été modifié pour désactiver l'authentification par certificat client pour le peer.
======
...
*Correction:*
Des mécanismes alternatifs fournis par Kubernetes, tels que l'utilisation d'OIDC, devraient être mis en œuvre à la place des jetons de compte de service.

==== 3.1.3 L'authentification par jeton de démarrage ne doit pas être utilisée pour les utilisateurs (Manuel)

*Résultat :* WARN

*Correction:*
Des mécanismes alternatifs fournis par Kubernetes, tels que l'utilisation d'OIDC, doivent être mis en œuvre à la place des jetons de démarrage.
...
Créez un fichier de stratégie d'audit pour votre cluster.
======

==== 3.2.2 Assurez-vous que la stratégie d'audit couvre les principales préoccupations en matière de sécurité (Manuel)

*Résultat :* WARN

*Correction:*
Examinez la stratégie d'audit fournie pour le cluster et assurez-vous qu'elle couvre au moins les domaines suivants,
...
* Modification des objets Pod et Deployment.
...
Par exemple, `chmod 600 /var/lib/rancher/rke2/agent/kubeproxy.kubeconfig`
======
...
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur chaque nœud de travail.
Par exemple, `chmod 600 /var/lib/rancher/rke2/agent/kubelet.kubeconfig`
======
...
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur chaque nœud de travail.
Par exemple, `chown root:root /var/lib/rancher/rke2/agent/kubelet.kubeconfig`
======
...
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur chaque nœud de travail.
Par exemple, `chmod 600 /var/lib/rancher/rke2/agent/client-ca.crt`
======
...
Exécutez la commande suivante pour modifier la propriété du fichier --client-ca-file. `chown root:root /var/lib/rancher/rke2/agent/client-ca.crt`
======
...
==== 4.1.10 Si le fichier de configuration kubelet config.yaml est utilisé, validez que la propriété du fichier est définie sur root:root (Automatisé)
...
Par défaut, RKE2 définit le paramètre --anonymous-auth sur false.
...
*Résultat attendu\&nbsp;:* '--streaming-connection-idle-timeout' est présent OU '--streaming-connection-idle-timeout' n'est pas présent
...
*Résultat attendu\&nbsp;:* '--make-iptables-util-chains' est présent OU '--make-iptables-util-chains' n'est pas présent
...
*Résultat attendu\&nbsp;:* '--event-qps' est présent OU '--event-qps' n'est pas présent
...
*Remédiation:*
Lorsque cela est possible, supprimez l'accès get, list et watch aux objets Secret dans le cluster.
...
RKE2 fait des exceptions pour les rôles de cluster suivants, qui sont nécessaires pour les opérations régulières :
...
Lorsqu'il est en mode cis, RKE2 désactive l'automountServiceAccountToken pour les espaces de noms :

* kube-public, default, kube-node-lease Si à la fois le ServiceAccount et le Pod .spec spécifient une valeur pour automountServiceAccountToken, la spécification du Pod prévaut.
 ...
 ** Le ServiceAccount est automountServiceAccountToken : true/non défini et le Pod est automountServiceAccountToken : false
======
...
==== 5.7.1 Créez des frontières administratives entre les ressources en utilisant des espaces de noms (Manuel)
