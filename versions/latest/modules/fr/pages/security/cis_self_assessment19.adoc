...
Ce guide est spécifique à la ligne de version *v1.27-1.32* de RKE2 et à la version *v1.9* du CIS Kubernetes Benchmark.
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud de plan de contrôle. Par exemple, `chmod 600 /var/lib/rancher/rke2/agent/pod-manifests/kube-apiserver.yaml`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud de plan de contrôle. Par exemple, `chown root:root /var/lib/rancher/rke2/agent/pod-manifests/kube-apiserver.yaml`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud de plan de contrôle. Par exemple, `chmod 600 /var/lib/rancher/rke2/agent/pod-manifests/kube-controller-manager.yaml`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud de plan de contrôle. Par exemple, `chown root:root /var/lib/rancher/rke2/agent/pod-manifests/kube-controller-manager.yaml`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud de plan de contrôle. Par exemple, `chmod 600 /var/lib/rancher/rke2/agent/pod-manifests/kube-scheduler.yaml`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud de plan de contrôle. Par exemple, `chown root:root /var/lib/rancher/rke2/agent/pod-manifests/kube-scheduler.yaml`
======
...
.Correction:
======
Si vous exécutez uniquement le maître sans rôle etcd, cette vérification n'est pas applicable. Si les rôles de plan de contrôle et etcd sont présents sur les mêmes nœuds mais que cette vérification est un avertissement, exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud du plan de contrôle. Par exemple, `chmod 600 /var/lib/rancher/rke2/agent/pod-manifests/etcd.yaml`
======
...
.Correction:
======
Si vous exécutez uniquement le maître sans rôle etcd, cette vérification n'est pas applicable. Si les rôles de plan de contrôle et etcd sont présents sur les mêmes nœuds mais que cette vérification est un avertissement, exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud du plan de contrôle. Par exemple, `chown root:root /var/lib/rancher/rke2/agent/pod-manifests/etcd.yaml`
======
...
.Correction:
======
Si vous exécutez uniquement le maître sans rôle etcd, cette vérification n'est pas applicable. Si les rôles de plan de contrôle et etcd sont présents sur les mêmes nœuds mais que cette vérification est un avertissement, alors sur le nœud serveur etcd, obtenez le répertoire de données etcd, passé comme argument --data-dir, à partir de la commande 'ps -ef | grep etcd'. Exécutez la commande ci-dessous (en fonction du répertoire de données etcd trouvé ci-dessus). Par exemple, `chown etcd:etcd /var/lib/rancher/rke2/server/db/etcd`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud de plan de contrôle. Par exemple, `chmod 600 /var/lib/rancher/rke2/server/cred/admin.kubeconfig`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud de plan de contrôle. Par exemple, `chown root:root /var/lib/rancher/rke2/server/cred/admin.kubeconfig`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud de plan de contrôle. Par exemple, `chmod 600 /var/lib/rancher/rke2/server/cred/scheduler.kubeconfig`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud de plan de contrôle. Par exemple, `chown root:root /var/lib/rancher/rke2/server/cred/scheduler.kubeconfig`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud de plan de contrôle. Par exemple, `chmod 600 /var/lib/rancher/rke2/server/cred/controller.kubeconfig`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud de plan de contrôle. Par exemple, `chown root:root /var/lib/rancher/rke2/server/cred/controller.kubeconfig`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud de plan de contrôle. Par exemple, `chown -R root:root /var/lib/rancher/rke2/server/tls`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur le nœud de plan de contrôle. Par exemple, `chmod -R 600 /var/lib/rancher/rke2/server/tls/*.key`
======
...
.Correction:
======
Par défaut, RKE2 définit l'argument --anonymous-auth sur false. Si cette vérification échoue, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et supprimez tout ce qui ressemble à ce qui suit.
----
...
.Correction:
======
Suivez la documentation et configurez des mécanismes alternatifs pour l'authentification. Si cette vérification échoue, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et supprimez tout ce qui ressemble à ce qui suit.
----
...
**Correction:** Par défaut, RKE2 ne définit pas DenyServiceExternalIPs. Pour activer ce drapeau, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml comme ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 fournit automatiquement le fichier de certificat CA kubelet, à /var/lib/rancher/rke2/server/tls/server-ca.crt. Si pour une raison quelconque vous devez fournir votre propre certificat CA, envisagez d'utiliser l'outil de ligne de commande de certificats rke2. Si cette vérification échoue, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et supprimez toutes les lignes comme ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 ne définit pas l'--authorization-mode sur AlwaysAllow. Si cette vérification échoue, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml, supprimez toutes les lignes comme ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 définit l'--authorization-mode sur Node et RBAC. Si cette vérification échoue, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml, assurez-vous que vous ne remplacez pas authorization-mode.
...
**Correction:** Suivez la documentation de Kubernetes et définissez les limites souhaitées dans un fichier de configuration. Ensuite, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et définissez les paramètres ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 ne définit pas le --enable-admission-plugins sur AlwaysAdmit. Si cette vérification échoue, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml, supprimez toutes les lignes comme ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 ne définit pas le --disable-admission-plugins sur quoi que ce soit. Si cette vérification échoue, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et supprimez toutes les lignes comme ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 définit le --enable-admission-plugins sur NodeRestriction. Vérifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et assurez-vous que vous ne remplacez pas les plugins d'admission. Si c'est le cas, incluez NodeRestriction dans la liste.
----
...
.Correction:
======
Par défaut, RKE2 définit l'argument --profiling sur false. Si cette vérification échoue, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et supprimez toutes les lignes comme ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 définit l'argument --audit-log-maxage sur 30 jours. Si vous souhaitez modifier cela, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml sur le nœud de contrôle et définissez le paramètre --audit-log-maxage sur un nombre de jours approprié, par exemple,
----
...
.Correction:
======
Par défaut, RKE2 définit l'argument --audit-log-maxbackup sur 10. Si vous souhaitez modifier cela, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml sur le nœud de contrôle et définissez le paramètre --audit-log-maxbackup sur une valeur appropriée. Exemples :
----
...
.Correction:
======
Par défaut, RKE2 définit l'argument --audit-log-maxsize sur 100 Mo. Si vous souhaitez modifier cela, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml sur le nœud de contrôle et définissez le paramètre --audit-log-maxsize sur une taille appropriée en Mo. Exemples :
----
...
.Correction:
======
Permissif, selon les directives CIS, "il est recommandé de définir cette limite selon ce qui est approprié et de ne changer la limite par défaut de 60 secondes que si nécessaire". Modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et définissez le paramètre ci-dessous si nécessaire. Exemples :
----
...
.Correction:
======
Par défaut, RKE2 ne définit pas l'argument --service-account-lookup. Modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et définissez le service-account-lookup. Exemples :
----
...
.Correction:
======
RKE2 génère automatiquement et définit le fichier de clé du compte de service. Il est situé à /var/lib/rancher/rke2/server/tls/service.key. Si cette vérification échoue, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et supprimez toutes les lignes comme ci-dessous.
----
...
.Correction:
======
RKE2 génère automatiquement et définit les fichiers de certificat et de clé etcd. Ils sont situés à /var/lib/rancher/rke2/server/tls/etcd/client.crt et /var/lib/rancher/rke2/server/tls/etcd/client.key. Si cette vérification échoue, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et supprimez toutes les lignes comme ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 fournit automatiquement le fichier d'autorité de certification du client. Il est généré et situé à /var/lib/rancher/rke2/server/tls/client-ca.crt. Si, pour une raison quelconque, vous devez fournir votre propre certificat CA, envisagez d'utiliser l'outil de ligne de commande rke2 certificate. Si cette vérification échoue, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et supprimez toutes les lignes comme ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 fournit automatiquement le fichier d'autorité de certification etcd. Il est généré et situé à /var/lib/rancher/rke2/server/tls/client-ca.crt. Si, pour une raison quelconque, vous devez fournir votre propre certificat CA, envisagez d'utiliser l'outil de ligne de commande rke2 certificate. Si cette vérification échoue, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et supprimez toutes les lignes comme ci-dessous.
----
...
.Correction:
======
RKE2 est toujours configuré pour chiffrer les secrets. Le chiffrement des secrets est géré avec l'outil de ligne de commande rke2 secrets-encrypt. Si nécessaire, vous pouvez trouver la configuration de chiffrement générée à /var/lib/rancher/rke2/server/cred/encryption-config.json.
...
.Correction:
======
RKE2 est toujours configuré pour utiliser le fournisseur de chiffrement aescbc pour chiffrer les secrets. Le chiffrement des secrets est géré avec l'outil de ligne de commande rke2 secrets-encrypt. Si nécessaire, vous pouvez trouver la configuration de chiffrement générée à /var/lib/rancher/rke2/server/cred/encryption-config.json.
...
.Correction:
======
Par défaut, le kube-apiserver RKE2 respecte ce test. Les modifications apportées à ces valeurs peuvent entraîner des régressions, assurez-vous donc que tous les clients apiserver prennent en charge la nouvelle configuration TLS avant de l'appliquer dans des déploiements en production. Si une configuration TLS personnalisée est requise, envisagez également de créer une version personnalisée de cette règle qui correspond à vos exigences. Si cette vérification échoue, supprimez toute configuration personnalisée autour de `tls-cipher-suites` ou mettez à jour le fichier /etc/rancher/rke2/config.yaml pour correspondre à la valeur par défaut en ajoutant ce qui suit{nbsp}: kube-apiserver-arg{nbsp}: - "tls-cipher-suites=TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384,TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384,TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256,TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256,TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305,TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305"
======
...
.Correction:
======
Par défaut, RKE2 définit le terminated-pod-gc-threshold à 1000. Si vous devez modifier cette valeur, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml sur le nœud de contrôle et définissez le --terminated-pod-gc-threshold sur une valeur appropriée.
----
...
.Correction:
======
Par défaut, RKE2 définit l'argument --profiling sur false. Si cette vérification échoue, éditez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et supprimez toutes les lignes comme ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 définit l'argument --use-service-account-credentials sur true. Si cette vérification échoue, éditez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et supprimez toutes les lignes comme ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 fournit automatiquement le fichier de clé privée du compte de service. Il est généré et situé à /var/lib/rancher/rke2/server/tls/service.current.key. Si cette vérification échoue, éditez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et supprimez toutes les lignes comme ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 fournit automatiquement le fichier CA racine. Il est généré et situé à /var/lib/rancher/rke2/server/tls/server-ca.crt. Si, pour une raison quelconque, vous devez fournir votre propre certificat CA, envisagez d'utiliser l'outil de ligne de commande de certificats rke2. Si cette vérification échoue, éditez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et supprimez toutes les lignes comme ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 ne définit pas le feature gate RotateKubeletServerCertificate. Si vous avez activé ce feature gate, vous devez le supprimer. Si cette vérification échoue, éditez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml, supprimez toutes les lignes comme ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 génère des fichiers de certificat et de clé pour etcd. Ils se trouvent dans /var/lib/rancher/rke2/server/tls/etcd/. Si cette vérification échoue, assurez-vous que le fichier de configuration /var/lib/rancher/rke2/server/db/etcd/config n'a pas été modifié pour utiliser des fichiers de certificat et de clé personnalisés.
...
.Correction:
======
Par défaut, RKE2 définit le paramètre --client-cert-auth sur 'true'. Si cette vérification échoue, assurez-vous que le fichier de configuration /var/lib/rancher/rke2/server/db/etcd/config n'a pas été modifié pour désactiver l'authentification par certificat client.
...
.Correction:
======
Par défaut, RKE2 génère des fichiers de certificat et de clé peer pour etcd. Ils se trouvent dans /var/lib/rancher/rke2/server/tls/etcd/. Si cette vérification échoue, assurez-vous que le fichier de configuration /var/lib/rancher/rke2/server/db/etcd/config n'a pas été modifié pour utiliser des fichiers de certificat et de clé peer personnalisés.
...
.Correction:
======
Par défaut, RKE2 définit le paramètre --peer-cert-auth sur 'true'. Si cette vérification échoue, assurez-vous que le fichier de configuration /var/lib/rancher/rke2/server/db/etcd/config n'a pas été modifié pour désactiver l'authentification par certificat client pour le peer.
...
.Correction:
======
Par défaut, RKE2 génère une autorité de certification unique pour etcd. Ceci est situé à /var/lib/rancher/rke2/server/tls/etcd/peer-ca.crt. Si cette vérification échoue, assurez-vous que le fichier de configuration /var/lib/rancher/rke2/server/db/etcd/config n'a pas été modifié pour utiliser une autorité de certification partagée.
...
**Correction:** Des mécanismes alternatifs fournis par Kubernetes, tels que l'utilisation d'OIDC, devraient être mis en œuvre à la place des certificats clients.

==== 3.1.2 L'authentification par jeton de compte de service ne doit pas être utilisée pour les utilisateurs (Manuel)

**Résultat :** WARN

**Correction:** Des mécanismes alternatifs fournis par Kubernetes, tels que l'utilisation d'OIDC, devraient être mis en œuvre à la place des jetons de compte de service.

==== 3.1.3 L'authentification par jeton de démarrage ne doit pas être utilisée pour les utilisateurs (Manuel)

**Résultat :** WARN

**Correction:** Des mécanismes alternatifs fournis par Kubernetes, tels que l'utilisation d'OIDC, doivent être mis en œuvre à la place des jetons de démarrage.
...
**Correction:** Examinez la politique d'audit fournie pour le cluster et assurez-vous qu'elle couvre au moins les domaines suivants{nbsp}: - Accès aux Secrets gérés par le cluster. Il convient de veiller à ne consigner que les métadonnées des demandes de Secrets, ConfigMaps et TokenReviews, afin d'éviter le risque de consigner des données sensibles. - Modification des objets Pod et Deployment. - Utilisation de `pods/exec`, `pods/portforward`, `pods/proxy` et `services/proxy`. Pour la plupart des demandes, il est recommandé de consigner au minimum au niveau des métadonnées (le niveau de journalisation le plus basique).
...
**Justification :** Le kubelet est géré par le processus RKE2. Il n'y a pas de fichier de service kubelet, toute la configuration est passée en tant qu'arguments à l'exécution.
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur chaque nœud de travail. Par exemple, `chmod 600 /var/lib/rancher/rke2/agent/kubeproxy.kubeconfig`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur chaque nœud de travail. Par exemple, `chown root:root /var/lib/rancher/rke2/agent/kubeproxy.kubeconfig`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur chaque nœud de travail. Par exemple, `chmod 600 /var/lib/rancher/rke2/agent/kubelet.kubeconfig`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur chaque nœud de travail. Par exemple, `chown root:root /var/lib/rancher/rke2/agent/kubelet.kubeconfig`
======
...
.Correction:
======
Exécutez la commande ci-dessous (en fonction de l'emplacement du fichier sur votre système) sur chaque nœud de travail. Par exemple, `chmod 600 /var/lib/rancher/rke2/agent/client-ca.crt`
======
...
**Justification :** Le kubelet est géré par le processus RKE2. Il n'y a pas de fichier de configuration kubelet, toute la configuration est passée en tant qu'arguments à l'exécution.
...
.Correction:
======
Par défaut, RKE2 définit --anonymous-auth sur false. Si cette vérification échoue, éditez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml, et supprimez toutes les lignes similaires à celles ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 ne définit pas le --authorization-mode sur AlwaysAllow. Si cette vérification échoue, éditez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml, et supprimez toutes les lignes similaires à celles ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 fournit automatiquement le certificat CA client pour le Kubelet. Il est généré et situé à /var/lib/rancher/rke2/agent/client-ca.crt
...
.Correction:
======
Par défaut, RKE2 définit le --read-only-port sur 0. Si vous l'avez défini sur une valeur différente, vous devez le remettre à 0. Modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml, supprimez toutes les lignes similaires à celles ci-dessous.
----
...
----
Ou, supprimez l'argument --make-iptables-util-chains pour laisser RKE2 utiliser la valeur par défaut. En fonction de votre système, redémarrez le service RKE2. Par exemple, systemctl restart rke2-server.service
...
**Justification :** Par défaut, RKE2 définit l'argument --hostname-override. Selon les directives CIS, cela est nécessaire pour se conformer aux fournisseurs de cloud qui exigent ce drapeau pour garantir que le nom d'hôte correspond aux noms des nœuds.
...
.Correction:
======
Par défaut, RKE2 fournit automatiquement le certificat TLS et la clé privée pour le Kubelet. Ils sont générés et situés à /var/lib/rancher/rke2/agent/serving-kubelet.crt et /var/lib/rancher/rke2/agent/serving-kubelet.key Si cette vérification échoue, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml et supprimez toutes les lignes similaires à celles ci-dessous.
----
...
.Correction:
======
Par défaut, RKE2 ne définit pas l'argument --rotate-certificates. Si cette vérification échoue, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml, et supprimez tout paramètre rotate-certificates. En fonction de votre système, redémarrez le service RKE2. Par exemple, systemctl restart rke2-server.service
...
.Correction:
======
Par défaut, RKE2 ne définit pas le feature gate RotateKubeletServerCertificate. Si cette vérification échoue, modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml, et supprimez tout paramètre RotateKubeletServerCertificate. En fonction de votre système, redémarrez le service RKE2. Par exemple, systemctl restart rke2-server.service
...
**Correction:** Modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml,
----
...
----
ou à un sous-ensemble de ces valeurs. En fonction de votre système, redémarrez le service RKE2. Par exemple, systemctl restart rke2-server.service

==== 4.2.13 Assurez-vous qu'une limite est définie sur les PID des pods (Manuel)

**Résultat :** WARN

**Correction:** Modifiez le fichier de configuration RKE2 /etc/rancher/rke2/config.yaml, définissez le paramètre suivant sur une valeur appropriée.
----
...
**Résultat attendu\&nbsp;:** 'cluster-admin' correspond à l'expression regex 'cluster-admin|helm-kube-system-rke2-.*'
...
.Correction:
======
Identifiez tous les clusterrolebindings au rôle cluster-admin. Vérifiez s'ils sont utilisés et s'ils ont besoin de ce rôle ou s'ils pourraient utiliser un rôle avec moins de privilèges. RKE2 accorde des exceptions aux clusterrolebindings helm-kube-system-rke2-* qui gèrent l'installation de tous les composants gérés par rke2. Lorsque cela est possible, liez d'abord les utilisateurs à un rôle de moindre privilège, puis supprimez le clusterrolebinding au rôle cluster-admin{nbsp}:
----
...
.Correction:
======
Créez des comptes de service explicites chaque fois qu'une charge de travail Kubernetes nécessite un accès spécifique au serveur API Kubernetes. Modifiez la configuration de chaque compte de service par défaut pour inclure cette valeur automountServiceAccountToken: false. Ou en utilisant kubectl{nbsp}:
----
...
.Correction:
======
Modifiez la définition des ServiceAccounts et des Pods qui n'ont pas besoin de monter des jetons de compte de service pour le désactiver, avec `automountServiceAccountToken: false`. RKE2 accorde des exceptions aux ServiceAccounts dans l'espace de noms kube-system, car ceux-ci sont censés contacter l'apiserver. Lorsqu'il est en mode cis, RKE2 désactive l'automountServiceAccountToken pour les espaces de noms{nbsp}: - kube-public, default, kube-node-lease. Si à la fois le ServiceAccount et le .spec du Pod définissent une valeur pour automountServiceAccountToken, celle du Pod prévaut. Condition{nbsp}: Le Pod est considéré comme conforme (vrai) lorsque{nbsp}: - le ServiceAccount est configuré avec automountServiceAccountToken défini sur false et le Pod avec automountServiceAccountToken défini sur false ou non défini{nbsp}; - ou lorsque le ServiceAccount est configuré avec automountServiceAccountToken défini sur true ou non défini et que le Pod est configuré avec automountServiceAccountToken défini sur false.
...
**Remédiation\&nbsp;:** Lorsque cela est possible, supprimez l'accès à la sous-ressource token des objets ServiceAccount.
...
**Correction:** Ajoutez des politiques à chaque espace de noms du cluster qui contient des charges de travail utilisateur pour restreindre l'admission de conteneurs utilisant des sections `hostPort`.
...
**Correction:** Suivez la documentation et créez des objets NetworkPolicy selon vos besoins.
...
**Correction:** Référez-vous aux options de gestion des secrets proposées par votre fournisseur de cloud ou une solution de gestion des secrets tierce.
...
**Correction:** Suivez la documentation Kubernetes et configurez la provenance des images.
...
**Correction:** Utilisez `securityContext` pour activer le profil seccomp docker/default dans vos définitions de Pod. Un exemple est le suivant{nbsp}: securityContext: seccompProfile: type: RuntimeDefault

==== 5.7.3 Appliquez SecurityContext à vos Pods et Conteneurs (Manuel)

**Résultat :** WARN

**Correction:** Suivez la documentation Kubernetes et appliquez des SecurityContexts à vos Pods. Pour une liste suggérée de SecurityContexts, vous pouvez vous référer au CIS Security Benchmark pour les conteneurs Docker.

==== 5.7.4 L'espace de noms par défaut ne doit pas être utilisé (Manuel)

**Résultat :** WARN

**Correction:** Assurez-vous que des espaces de noms sont créés pour permettre une séparation appropriée des ressources Kubernetes et que toutes les nouvelles ressources sont créées dans un espace de noms spécifique.+++</none>++++++</none>++++++</none>++++++</none>++++++</none>+++
