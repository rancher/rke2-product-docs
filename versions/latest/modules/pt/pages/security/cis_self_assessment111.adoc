...
Este documento é um complemento ao guia de proteção do RKE2. O guia de proteção fornece orientações prescritivas para a proteção de uma instalação de produção do RKE2, e este guia de referência tem como objetivo ajudá-lo a avaliar o nível de segurança do cluster protegido em relação a cada controle no CIS Kubernetes Benchmark. Deve ser utilizado por operadores de RKE2, equipes de segurança, auditores e tomadores de decisão.

Este guia é específico para a linha de versões *v1.29-1.34* do RKE2 e a versão *v1.11* do CIS Kubernetes Benchmark.

Para mais informações sobre cada controle, incluindo justificativas detalhadas e descrições de verificações, você pode consultar a seção correspondente do CIS Kubernetes Benchmark v1.8. Você pode baixar o benchmark, após criar uma conta gratuita, no https://www.cisecurity.org/benchmark/kubernetes/[Centro para Segurança da Internet (CIS)].
...
* *AVISO* - O controle é manual (pontuação: falso) no benchmark do CIS e depende da intervenção manual do operador. A seção de remediação fornecerá orientações sobre como alcançar um resultado de APROVADO.
...
==== 1.1.12 Assegure-se de que a propriedade do diretório de dados etcd esteja definida como etcd:etcd (Manual)
...
*Resultado Esperado:* as permissões devem estar definidas como 644 ou serem mais restritivas.
...
Por padrão, o RKE2 define as permissões do arquivo de certificado PKI como 644, permissões mais restritivas, como 600, são suportadas.
======
...
Se esta verificação falhar, edite o arquivo de configuração do RKE2 /etc/rancher/rke2/config.yaml e remova quaisquer linhas como as abaixo.
...
Se esta verificação falhar, edite o arquivo de configuração do RKE2 /etc/rancher/rke2/config.yaml, remova quaisquer linhas como as abaixo.
...
Se esta verificação falhar, edite o arquivo de configuração do RKE2 /etc/rancher/rke2/config.yaml, assegurando que você não está sobrescrevendo o authorization-mode.
======
...
Por padrão, o RKE2 não define o --disable-admission-plugins para nada.
...
Por padrão, o RKE2 configura o --enable-admission-plugins para NodeRestriction.
...
Eles são gerados e localizados em /var/lib/rancher/rke2/server/tls/serving-kube-apiserver.crt e /var/lib/rancher/rke2/server/tls/serving-kube-apiserver.key. Se esta verificação falhar, edite o arquivo de configuração do RKE2 /etc/rancher/rke2/config.yaml e remova quaisquer linhas como abaixo.
...
Se por algum motivo você precisar fornecer seu próprio certificado CA, considere usar a ferramenta de linha de comando rke2 certificate.
Se esta verificação falhar, edite o arquivo de configuração do RKE2 /etc/rancher/rke2/config.yaml e remova quaisquer linhas como abaixo.
...
Se esta verificação falhar, remova qualquer configuração personalizada em torno de `tls-cipher-suites` ou atualize o arquivo /etc/rancher/rke2/config.yaml para corresponder ao padrão, adicionando o seguinte:
kube-apiserver-arg:
...
Edite o arquivo de configuração do RKE2 /etc/rancher/rke2/config.yaml e defina o parâmetro --service-account-extend-token-expiration como false antes de recarregar o rke2, conforme abaixo:
 kube-apiserver-arg:
...
Por padrão, o RKE2 define um terminated-pod-gc-threshold de 1000.
...
Se por algum motivo você precisar fornecer seu próprio CA, considere usar a ferramenta de linha de comando de certificado do rke2.
Se essa verificação falhar, edite o arquivo de configuração do RKE2 /etc/rancher/rke2/config.yaml e remova quaisquer linhas como abaixo.
...
Por padrão, o RKE2 gera arquivos de certificado e chave para o etcd.
Estes estão localizados em /var/lib/rancher/rke2/server/tls/etcd/.
Se essa verificação falhar, certifique-se de que o arquivo de configuração /var/lib/rancher/rke2/server/db/etcd/config não foi modificado para usar arquivos de certificado e chave personalizados.
======
...
*Resultado esperado:* 'ETCD_CLIENT_CERT_AUTH' está presente OU '.client-transport-security.client-cert-auth' é igual a 'verdadeiro'
...
Se esta verificação falhar, edite o arquivo de especificação do pod etcd /var/lib/rancher/rke2/server/db/etcd/config no nó mestre e remova o parâmetro --auto-tls ou defina-o como falso. client-transport-security: auto-tls: falso
======
...
Por padrão, o RKE2 gera arquivos de certificado e chave de par para o etcd.
Estes estão localizados em /var/lib/rancher/rke2/server/tls/etcd/.
Se esta verificação falhar, certifique-se de que o arquivo de configuração /var/lib/rancher/rke2/server/db/etcd/config não foi modificado para usar arquivos de certificado e chave de par personalizados.
======
...
*Resultado esperado:* 'ETCD_PEER_CLIENT_CERT_AUTH' está presente OU '.peer-transport-security.client-cert-auth' é igual a 'verdadeiro'
...
Se esta verificação falhar, certifique-se de que o arquivo de configuração /var/lib/rancher/rke2/server/db/etcd/config não foi modificado para desabilitar a autenticação de certificado de cliente de par.
======
...
Por padrão, o RKE2 gera uma autoridade certificadora única para etcd.
Isso está localizado em /var/lib/rancher/rke2/server/tls/etcd/peer-ca.crt.
Se esta verificação falhar, certifique-se de que o arquivo de configuração /var/lib/rancher/rke2/server/db/etcd/config não foi modificado para usar uma autoridade certificadora compartilhada.
======
...
*Remediação:*
Mecanismos alternativos fornecidos pelo Kubernetes, como o uso de OIDC, devem ser implementados no lugar de tokens de conta de serviço.

==== 3.1.3 A autenticação de token de bootstrap não deve ser usada para usuários (Manual)
...
==== 3.2.2 Certifique-se de que a política de auditoria abranja preocupações de segurança chave (Manual)

*Resultado:* AVISO

*Remediação:*
Revise a política de auditoria fornecida para o cluster e certifique-se de que ela abranja pelo menos as seguintes áreas,
...
== 4 Configuração de Segurança do Nó Worker
...
==== 4.1.9 Se o arquivo de configuração kubelet config.yaml estiver sendo usado, valide as permissões definidas como 600 ou mais restritivas (Automatizado)
...
*Resultado Esperado:* '.authentication.anonymous.enabled' é igual a 'falso'
...
Por padrão, o RKE2 define o --anonymous-auth como false.
...
*Resultado Esperado:* '.streamingConnectionIdleTimeout' não é igual a '0' OU '.streamingConnectionIdleTimeout' não está presente
...
*Resultado Esperado:* '.makeIPTablesUtilChains' está presente OU '.makeIPTablesUtilChains' não está presente
...
*Resultado Esperado:* '.eventRecordQPS' está presente OU '.eventRecordQPS' não está presente
...
*Resultado Esperado:* '.tlsCertFile' está presente E '.tlsPrivateKeyFile' está presente
...
Se estiver usando um arquivo de configuração do RKE2 /etc/rancher/rke2/config.yaml, edite o arquivo para definir `seccomp-default` para kubelet-arg:
...
[cols=2*]
|===
| *Resultado Esperado:* 'cluster-admin' corresponde à expressão regex 'cluster-admin
...
Identifique todos os clusterrolebindings para o papel de cluster-admin. Verifique se estão sendo usados e se precisam desse papel ou se podem usar um papel com menos privilégios. O RKE2 dá exceções aos clusterrolebindings helm-kube-system-rke2-* que gerenciam a instalação de todos os componentes gerenciados pelo rke2.
...
O RKE2 dá exceções às ServiceAccounts no namespace kube-system, pois estas são esperadas para contatar o apiserver.
...
Auditoria: a auditoria lista todos os contêineres dos pods para recuperar o valor .securityContext.privileged.
...
Auditoria: a auditoria recupera as capacidades adicionadas de cada contêiner dos pods.
