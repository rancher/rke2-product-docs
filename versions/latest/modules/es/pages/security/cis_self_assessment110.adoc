...
Esta guía es específica para la línea de lanzamiento *v1.28-1.31* de RKE2 y la versión *v1.10* del CIS Kubernetes Benchmark.
...
Si por alguna razón necesitas proporcionar tu propio certificado CA, considera utilizar las herramientas shell de certificados de rke2.
...
Por defecto, RKE2 no establece el --authorization-mode en AlwaysAllow.
...
Por defecto, RKE2 no establece el --habilitar-plugins-de-admisión en AlwaysAdmit.
Si esta verificación falla, edita el archivo de configuración de RKE2 /etc/rancher/rke2/config.yaml, elimina cualquier línea como la siguiente.
...
Edita el archivo de configuración de RKE2 /etc/rancher/rke2/config.yaml en el nodo de control y establece el parámetro --habilitar-plugins-de-admisión para incluir AlwaysPullImages. --habilitar-plugins-de-admisión=...,AlwaysPullImages,...
...
Por defecto, RKE2 no establece el --deshabilitar-plugins-de-admisión en nada.
...
Por defecto, RKE2 establece el --habilitar-plugins-de-admisión en NodeRestriction.
...
Si por alguna razón necesitas proporcionar tu propio certificado CA, considera utilizar la herramienta shell de rke2.
Si esta verificación falla, edita el archivo de configuración de RKE2 /etc/rancher/rke2/config.yaml y elimina cualquier línea como la siguiente.
...
Por defecto, RKE2 establece un umbral de terminated-pod-gc-threshold de 1000.
Si necesitas cambiar este valor, edita el archivo de configuración de RKE2 /etc/rancher/rke2/config.yaml en el nodo de control y establece el --terminated-pod-gc-threshold a un umbral adecuado,
...
Por defecto, RKE2 proporciona automáticamente el archivo de CA raíz.
Se genera y se encuentra en /var/lib/rancher/rke2/server/tls/server-ca.crt.
Si por alguna razón necesitas proporcionar tu propio certificado CA, considera usar la herramienta de línea de comandos de rke2.
Si esta verificación falla, edita el archivo de configuración de RKE2 /etc/rancher/rke2/config.yaml y elimina cualquier línea como la siguiente.
...
Si esta verificación falla, asegúrate de que el archivo de configuración /var/lib/rancher/rke2/server/db/etcd/config no ha sido modificado para deshabilitar la autenticación mediante certificado de cliente.
======
...
Si esta comprobación falla, edita el archivo de especificación del pod etcd /var/lib/rancher/rke2/server/db/etcd/config en el nodo maestro y elimina el parámetro --auto-tls o configúralo como falso. +
client-transport-security: +
  auto-tls: falso
======
...
Por defecto, RKE2 establece el parámetro --peer-cert-auth en verdadero.
Si esta comprobación falla, asegúrate de que el archivo de configuración /var/lib/rancher/rke2/server/db/etcd/config no ha sido modificado para desactivar la autenticación mediante certificado de cliente de par.
======
...
Si esta comprobación falla, edita el archivo de especificación del pod etcd /var/lib/rancher/rke2/server/db/etcd/config en el nodo maestro y elimina el parámetro --peer-auto-tls o configúralo como falso. +
peer-transport-security: +
  auto-tls: falso
======
...
Por defecto, RKE2 genera una autoridad de certificación única para etcd.
Esto se encuentra en /var/lib/rancher/rke2/server/tls/etcd/peer-ca.crt.
Si esta comprobación falla, asegúrate de que el archivo de configuración /var/lib/rancher/rke2/server/db/etcd/config no ha sido modificado para utilizar una autoridad de certificación compartida.
======
...
==== 3.1.2 La autenticación con token de cuenta de servicio no debe ser utilizada para usuarios (Manual)

*Resultado:* WARN (ADVERTENCIA)

*Solución:*
Los mecanismos alternativos proporcionados por Kubernetes, como el uso de OIDC, deben ser implementados en lugar de tokens de cuenta de servicio.

==== 3.1.3 La autenticación con token de inicio no debe ser utilizada para los usuarios (Manual)
...
* Acceso a secretos gestionados por el clúster. Se debe tener cuidado de solo registrar metadatos para las solicitudes a secretos, ConfigMaps y TokenReviews, para evitar el riesgo de registrar datos sensibles.
* Modificación de objetos Pod y Deployment.
...
Por defecto, RKE2 establece el --anonymous-auth en falso.
...
Se generan y se encuentran en /var/lib/rancher/rke2/agent/serving-kubelet.crt y /var/lib/rancher/rke2/agent/serving-kubelet.key. Si esta comprobación falla, edita el archivo de configuración de RKE2 /etc/rancher/rke2/config.yaml y elimina cualquier línea similar a la siguiente.
...
Si esta comprobación falla, edita el archivo de configuración de RKE2 /etc/rancher/rke2/config.yaml. Elimina cualquier parámetro rotate-certificates.
Según tu sistema, reinicia el servicio RKE2. Por ejemplo, systemctl restart rke2-server.service
======
...
Por defecto, RKE2 no establece el feature gate RotateKubeletServerCertificate.
Si esta comprobación falla, edita el archivo de configuración de RKE2 /etc/rancher/rke2/config.yaml y elimina cualquier parámetro RotateKubeletServerCertificate.
Según tu sistema, reinicia el servicio RKE2. Por ejemplo, systemctl restart rke2-server.service
======
...
[cols=2*]
|===
| *Resultado Esperado:* 'cluster-admin' coincidente con la expresión regex 'cluster-admin
...
RKE2 otorga excepciones a los siguientes pods, que son necesarios para las operaciones regulares:
...
RKE2 otorga excepciones a los siguientes pods en el espacio de nombres kube-system, que son necesarios para las operaciones regulares:
