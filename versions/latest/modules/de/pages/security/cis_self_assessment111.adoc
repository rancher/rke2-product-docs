= CIS 1.11 Selbstbewertungsanleitung
...
* *Nicht anwendbar* - Die Kontrolle ist nicht anwendbar (Typ: überspringen) für RKE2, aufgrund der Art und Weise, wie es konzipiert ist, zu arbeiten. Der Abschnitt zur Begründung wird erklären, warum dies so ist.
* *WARNUNG* - Die Kontrolle ist manuell (bewertet: falsch) im CIS-Benchmark und hängt von der manuellen Intervention des Betreibers ab. Der Abschnitt zur Behebung wird Anleitungen geben, wie man ein BESTANDEN-Ergebnis erzielt.
...
Führen Sie den folgenden Befehl (basierend auf dem Dateispeicherort auf Ihrem System) auf dem Knoten der Steuerungsebene aus.
...
Wenn die Rollen Steuerung und etcd auf denselben Knoten vorhanden sind, aber diese Überprüfung eine Warnung ist, dann führen Sie den folgenden Befehl (basierend auf dem Dateispeicherort auf Ihrem System) auf dem Steuerungsknoten aus.
Zum Beispiel,
...
==== 1.1.12 Stellen Sie sicher, dass der Eigentümer des etcd-Datenverzeichnisses (Verzeichnis) auf etcd:etcd gesetzt ist (Manuell)
...
Führen Sie den folgenden Befehl (basierend auf dem Dateispeicherort auf Ihrem System) auf dem Steuerungsknoten aus.
Zum Beispiel, `chmod -R 644 /var/lib/rancher/rke2/server/tls/*.crt` +
Standardmäßig setzt RKE2 die Berechtigungen der PKI-Zertifikatdateien auf 644, restriktivere Berechtigungen wie 600 werden unterstützt.
======
...
Wenn diese Überprüfung fehlschlägt, bearbeiten Sie die RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml und entfernen Sie alle Zeilen, die dem Folgenden ähneln.
...
Standardmäßig setzt RKE2 --authorization-mode auf Node und RBAC.
Wenn diese Überprüfung fehlschlägt, bearbeiten Sie die RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml und stellen Sie sicher, dass Sie authorization-mode nicht überschreiben.
======
...
Wenn Sie dies ändern möchten, bearbeiten Sie die RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml +
auf dem Steuerungsknoten und setzen Sie den Parameter --audit-log-maxage auf eine angemessene Anzahl von Tagen, zum Beispiel:
...
Wenn Sie dies ändern möchten, bearbeiten Sie die RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml +
auf dem Steuerungsknoten und setzen Sie den Parameter --audit-log-maxbackup auf einen geeigneten Wert.
Beispiel:
...
Wenn Sie dies ändern möchten, bearbeiten Sie die RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml +
auf dem Steuerungsknoten und setzen Sie den Parameter --audit-log-maxsize auf eine angemessene Größe in MB.
Beispiel:
...
und setzen Sie den untenstehenden Parameter, falls erforderlich. Beispiel:
...
Bearbeiten Sie die RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml und setzen Sie das service-account-lookup. Beispiel:
...
Wenn diese Überprüfung fehlschlägt, bearbeiten Sie die RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml +
und entfernen Sie alle Zeilen wie unten.
...
Wenn diese Überprüfung fehlschlägt, bearbeiten Sie die RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml +
und entfernen Sie alle Zeilen, die dem Folgenden ähneln.
...
Wenn diese Überprüfung fehlschlägt, entfernen Sie alle benutzerdefinierten Konfigurationen rund um `tls-cipher-suites` oder aktualisieren Sie die Datei /etc/rancher/rke2/config.yaml, um den Standardeinstellungen zu entsprechen, indem Sie Folgendes hinzufügen: kube-apiserver-arg:
...
Bearbeiten Sie die RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml und setzen Sie den --service-account-extend-token-expiration Parameter auf false, bevor Sie rke2 neu laden, wie folgt: kube-apiserver-arg:

* "service-account-extend-token-expiration=false" Standardmäßig ist dieser Parameter auf true gesetzt.
======
...
Standardmäßig setzt RKE2 das Argument --use-service-account-credentials auf true.
Wenn diese Überprüfung fehlschlägt, bearbeiten Sie die RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml und entfernen Sie alle Zeilen wie unten.
...
Standardmäßig setzt RKE2 das Argument --bind-address auf 127.0.0.1. Wenn diese Überprüfung fehlschlägt, bearbeiten Sie die RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml und entfernen Sie alle Zeilen wie unten.
...
Wenn diese Überprüfung fehlschlägt, stellen Sie sicher, dass die Konfigurationsdatei /var/lib/rancher/rke2/server/db/etcd/config
nicht geändert wurde, um benutzerdefinierte Zertifikats- und Schlüsseldateien zu verwenden.
======
...
Standardmäßig setzt RKE2 den Parameter --client-cert-auth auf true.
Wenn diese Überprüfung fehlschlägt, stellen Sie sicher, dass die Konfigurationsdatei /var/lib/rancher/rke2/server/db/etcd/config
nicht geändert wurde, um die Authentifizierung mit Client-Zertifikaten zu deaktivieren.
======
...
Wenn diese Überprüfung fehlschlägt, bearbeiten Sie die Spezifikationsdatei des etcd-Pods /var/lib/rancher/rke2/server/db/etcd/config
auf dem Master-Knoten und entfernen Sie entweder den --auto-tls-Parameter oder setzen Sie ihn auf false. client-transport-security: auto-tls: false
======
...
Standardmäßig generiert RKE2 Peer-Zertifikate und Schlüsseldateien für etcd.
Diese befinden sich in /var/lib/rancher/rke2/server/tls/etcd/.
Wenn diese Überprüfung fehlschlägt, stellen Sie sicher, dass die Konfigurationsdatei /var/lib/rancher/rke2/server/db/etcd/config
nicht geändert wurde, um benutzerdefinierte Peer-Zertifikate und Schlüsseldateien zu verwenden.
======
...
Wenn diese Überprüfung fehlschlägt, stellen Sie sicher, dass die Konfigurationsdatei /var/lib/rancher/rke2/server/db/etcd/config
nicht geändert wurde, um die Authentifizierung mit Peer-Client-Zertifikaten zu deaktivieren.
======
...
Standardmäßig generiert RKE2 eine einzigartige Zertifizierungsstelle für etcd.
Diese befindet sich unter /var/lib/rancher/rke2/server/tls/etcd/peer-ca.crt.
Wenn diese Überprüfung fehlschlägt, stellen Sie sicher, dass die Konfigurationsdatei /var/lib/rancher/rke2/server/db/etcd/config nicht geändert wurde, um eine gemeinsame Zertifizierungsstelle zu verwenden.
======

== 3 Steuerungsebenenkonfiguration
...
==== 3.1.2 Die Authentifizierung mit Service-Account-Token sollte nicht für Benutzer verwendet werden (Manuell)

*Ergebnis:* WARN

*Behebung:*
Alternative Mechanismen, die von Kubernetes bereitgestellt werden, wie die Verwendung von OIDC, sollten anstelle von Service-Account-Token implementiert werden.

==== 3.1.3 Die Authentifizierung mit Bootstrap-Token sollte nicht für Benutzer verwendet werden (Manuell)
...
*Behebung:*
Überprüfen Sie die für den Cluster bereitgestellte Audit-Richtlinie und stellen Sie sicher, dass sie mindestens die folgenden Bereiche abdeckt:
...
Führen Sie den folgenden Befehl aus, um den Besitzer der --client-ca-Datei zu ändern. `chown root:root /var/lib/rancher/rke2/agent/client-ca.crt`
======

==== 4.1.9 Wenn die kubelet-Konfigurationsdatei config.yaml verwendet wird, überprüfen Sie, ob die Berechtigungen auf 600 oder restriktiver gesetzt sind (Automatisiert)
...
==== 4.1.10 Wenn die kubelet-Konfigurationsdatei config.yaml verwendet wird, überprüfen Sie, ob der Dateibesitzer auf root:root gesetzt ist (Automatisiert)
...
*Erwartetes Ergebnis:* '.authentication.anonymous.enabled' ist gleich 'false'
...
Wenn diese Überprüfung fehlschlägt, bearbeiten Sie die RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml und entfernen Sie alle Zeilen, die ähnlich wie unten sind.
...
*Erwartetes Ergebnis:* '.authentication.x509.clientCAFile' ist vorhanden
...
Standardmäßig stellt RKE2 automatisch das Client-CA-Zertifikat für das Kubelet zur Verfügung.
...
Standardmäßig setzt RKE2 das --read-only-port auf 0. Wenn Sie dies auf einen anderen Wert gesetzt haben, sollten Sie es wieder auf 0 setzen. Bearbeiten Sie die RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml und entfernen Sie alle Zeilen, die ähnlich wie unten sind.
...
*Erwartetes Ergebnis:* '.streamingConnectionIdleTimeout' ist nicht gleich '0' ODER '.streamingConnectionIdleTimeout' ist nicht vorhanden
...
*Erwartetes Ergebnis:* '.makeIPTablesUtilChains' ist vorhanden ODER '.makeIPTablesUtilChains' ist nicht vorhanden
...
Oder entfernen Sie das --make-iptables-util-chains-Argument, damit RKE2 den Standardwert verwenden kann.
Je nach System starten Sie den RKE2-Dienst neu. Zum Beispiel, systemctl restart rke2-server.service
======
...
*Erwartetes Ergebnis:* '.eventRecordQPS' ist vorhanden ODER '.eventRecordQPS' ist nicht vorhanden
...
*Erwartetes Ergebnis:* '.tlsCertFile' ist vorhanden UND '.tlsPrivateKeyFile' ist vorhanden
...
Wenn diese Überprüfung fehlschlägt, bearbeiten Sie die RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml und entfernen Sie jeden rotate-certificates-Parameter.
Je nach System starten Sie den RKE2-Dienst neu. Zum Beispiel, systemctl restart rke2-server.service
======
...
Standardmäßig setzt RKE2 das RotateKubeletServerCertificate Feature-Gate nicht.
Wenn diese Überprüfung fehlschlägt, bearbeiten Sie die RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml und entfernen Sie jeden RotateKubeletServerCertificate-Parameter.
Je nach System starten Sie den RKE2-Dienst neu. Zum Beispiel, systemctl restart rke2-server.service
======
...
*Behebung:*
Wenn aktiviert, verwendet das Kubelet standardmäßig das RuntimeDefault seccomp-Profil, das von der Container-Laufzeit definiert wird, anstatt den Unconfined (seccomp deaktiviert) Modus (Standard) zu verwenden.
Wenn Sie eine RKE2-Konfigurationsdatei /etc/rancher/rke2/config.yaml verwenden, bearbeiten Sie die Datei, um `seccomp-default` auf kubelet-arg: zu setzen:
...
[cols=2*]
|===
| *Erwartetes Ergebnis:* 'cluster-admin' wird durch den regulären Ausdruck 'cluster-admin' gefunden
...
RKE2 gibt Ausnahmen für die ServiceAccounts im kube-system-Namespace, da diese erwartet werden, um den apiserver zu kontaktieren.
Wenn im CIS-Modus deaktiviert RKE2 das automountServiceAccountToken für die Namespaces:
...
* rke2-ingress-ngninx-controller, coredns. Audit: Das Audit ruft die hinzugefügten Fähigkeiten der Container jedes Pods ab.
...
vorgeschlagene Liste von SecurityContexts können Sie sich an den CIS Security Benchmark für Docker-Container wenden.
